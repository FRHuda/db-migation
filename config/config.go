...
type DbMigration struct {
	DSNMigrationLocal      string `env:"POST_MIGRATE_DSN_STAGING,default=host=localhost port=5432 database=post user=postgres password=postgres sslmode=disable"`
	DSNMigrationStaging    string `env:"POST_MIGRATE_DSN_STAGING,default=host=localhost port=5432 database=post user=postgres password=postgres sslmode=disable"`
